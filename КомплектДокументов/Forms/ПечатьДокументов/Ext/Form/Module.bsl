...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПараметрыПечати.ВывестиПечатьИПодписи Тогда
		ДобавитьПодписиИПечати(КоллекцияПечатныхФорм, ОбъектыПечати);
	Иначе
...
Процедура ДобавитьПодписиИПечати(ПечатныеФормы, ОбъектыПечати)
	...
	Для Каждого ПечатнаяФорма Из ПечатныеФормы Цикл
		УправлениеПечатью.ДобавитьПодписьИПечать(ПечатнаяФорма.ТабличныйДокумент, ПодписиИПечатиОбластей);
	КонецЦикла;
