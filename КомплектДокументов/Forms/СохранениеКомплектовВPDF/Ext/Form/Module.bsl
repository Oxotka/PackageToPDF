...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОтсортироватьПечатныеФормыПоУмолчанию();
	
КонецПроцедуры

&НаСервере
Процедура ОтсортироватьПечатныеФормыПоУмолчанию()
	...
	Если КомплектПечатныхФорм.НайтиСтроки(Новый Структура("Печатать", Истина)).Количество() > 0 Тогда
		...
		Если СразуСохранять Тогда
			...
			Для Каждого Документ Из Объекты.ВыгрузитьЗначения() Цикл
				...
				СохранитьФайлПакета(Документ, ПечатныеФормы.ТабличныеДокументы);
...
&НаСервере
Процедура СохранитьФайлПакета(Документ, ТабличныеДокументы)
	...
	ИмяФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПутьКФайлам) + ИмяДокумента(Документ);
...
Функция ИмяДокумента(Документ)
	...
	Возврат СтрШаблон("тн № %1 от %2 - %3.pdf", 
